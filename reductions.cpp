...
int AlmostTwin_reduction::reduce(Graph* g) {
    ...
    for (int i = offset; i < copy.size()-1; i++) {
        for (int j = i+1; j < copy.size(); j++) {
            ...
            //this probably not needed because edge size is sorted
            ...
            if (less->edges.size()+1 == more->edges.size()) {
                ...
                if (less->edges[0].neighbour_id == more->edges[0].neighbour_id) {
                    ...
                    if (tmp) {
                        ...
                        g->makeNodeInvisibleMarlon(less->order);
                        if (j < copy.size()-1) {
                            i++;
                        }
                    }
...
int Domination_reduction::reduce(Graph* g, int a) {
    ...
    while (p <= 10) {
        for (int i = g->getOffsetVisibleOrderNodes(); i < g->getSizeOfOrder()-1; i++) {
            for (int j = i+1; j < g->getSizeOfOrder(); j++) {
                //set node with less nodes
                Node* less = g->getNodeByOrder(j);
                Node* more = g->getNodeByOrder(i);
                if (g->getNodeByOrder(i)->edges.size() < g->getNodeByOrder(j)->edges.size()) {
                        less = g->getNodeByOrder(i); //i has less nodes
                        more = g->getNodeByOrder(j);
                    }

                if (less->offset_visible_nodes == less->edges.size() || 
                    more->offset_visible_nodes == more->edges.size()) {
                        continue;
                    }

                if (more->edges.size() == less->edges.size()+p) {
                    
                    //find first identical node
                    for (int k = 0; k <= more->edges.size() - less->edges.size(); k++) {
                        bool tmp = false;
                        if (more->edges[k].neighbour_id != less->edges[0].neighbour_id) {
                            continue;
                        }
                        ...
                        for (int l = 1; l < less->edges.size(); l++) { 
                            ...
                            if (more->edges[k+l].neighbour_id != less->edges[l].neighbour_id
                                || less->edges[l].neighbour_id != less->edges[l-1].neighbour_id+1) {
                                break;
                            }
                           //found Domination
                        ...
                        if (tmp) {
                            ...
                            if (k <= more->edges.size()-k-less->edges.size()) {
                                ...
                                restore_vec.push_back({more->id, less->id, 0, (int)less->edges.size(), less->edges[0].neighbour_id});
                            } else {
                                //place it on the right side
                                restore_vec.push_back({more->id, less->id, 1, (int)less->edges.size(), less->edges[0].neighbour_id});
                                }
                                //SET EDGE WEIGHTS
