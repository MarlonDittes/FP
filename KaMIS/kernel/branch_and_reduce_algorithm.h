...
private:
	...
	struct graph_status {
		size_t n0 = 0;
		size_t n1 = 0;
		size_t remaining_nodes_fixed = 0;
		size_t remaining_nodes_moveable = 0;
		int crossings = 0;
		Graph graph;
		...
		graph_status(Graph& G) :
			n0(G.getN0()), n1(G.getN1()), remaining_nodes_fixed(n0), remaining_nodes_moveable(n1), graph(G), crossings(0),
			node_status(n0 + n1, IS_status::not_set),
			folded_queue(n0 + n1), branching_queue(n0 + n1), modified_queue(n0 + n1 + 1) {
		}
	};
